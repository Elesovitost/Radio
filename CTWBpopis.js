...
var hodnoceniOptions = [
    ...
    { text: "spíše ben.", value: ": nemá charakter malignity", valueRPH: ": v.s. zánětlivá aktivace"},
    { text: "nerozhodný", value: ": nespecifický nález", valueRPH: ": nespecifický nález"},
    { text: "spíše mal.", value: ": suspektní z malignity", valueRPH: ": suspektní z meta"},
    { text: "maligní", value:": charakteru malignity", valueRPH: ": charakteru meta"},
...
function checkViability(text) {
  const keywords = ['ožisko', 'xpanze', 'odul', 'pacita', 'onsolidace',  'olekce',  'asa',  'nfiltrace',  'asa'];
...
if (ParotidavidLesionR && ParotidavidLesionL) {
  NeckParotidText += "noduly oboustranně, ";
  NeckParotidRes   = "Noduly obou parotidy ke sledování / dovyšetření. ";
} else {
  if (ParotidavidLesionR) {
    NeckParotidText += "nodul vpravo, ";
    NeckParotidRes   = "Nodul pravé parotidy ke sledování / dovyšetření. ";
  }
  if (ParotidavidLesionL) {
    NeckParotidText += "nodul vlevo, ";
    NeckParotidRes   = "Nodul levé parotidy ke sledování / dovyšetření. ";
  }
}

if (ParotidresectionR && ParotidresectionL) {
  NeckParotidText += "chybí po bilat. resekci, ";
} else {
...
var NeckThyroidText = ""; var NeckThyroidRes = "";
  ...
  if (ThyroidenlargementR && ThyroidenlargementL) { NeckThyroidText += "zvětšená, "; NeckThyroidRes += "Zvětšená štítná žláza v rámci thyreopatie. "; }
  else {
    if (ThyroidenlargementR) { NeckThyroidText += "zvětšený pravý lalok, "; NeckThyroidRes = "Zvětšený pravý lalok štítnice. "; }
...
CTWBNAMEText.value = nazev;

CTWBINDText.value = indikace; document.getElementById("indikace").addEventListener("input", updateTexts);
...
// úpravy výstupu podle rozsahu (KRK / HRUDNÍK / BŘICHO)
(function adjustPOPText() {
  ...
  // pomocné regexy (multiline, CRLF i LF)
  const KRK_RE      = /(^|\r?\n)Krk:\s/;
  const HRUDNIK_RE  = /(^|\r?\n)Hrudník:\s/;
  const BRICHO_RE   = /(^|\r?\n)Břicho:\s/;
  const SKELET_RE   = /(^|\r?\n)Skelet a měkké tkáně:\s/;

  const renameLine = (re, label) => { t = t.replace(re, `$1${label}`); };

  if (count === 1 && k) {
    // jen KRK: Hrudník -> "v zachyceném rozsahu", smazat celý blok Břicho až po Skelet (Skelet ponechat)
    renameLine(HRUDNIK_RE, 'Hrudník v zachyceném rozsahu: ');
    t = t.replace(/(\r?\n)Břicho:[\s\S]*?(?=\r?\nSkelet a měkké tkáně:\s)/, '$1');
  } else if (count === 1 && h) {
    // jen HRUDNÍK: Krk -> "v zachyceném rozsahu", Břicho -> "Epigastrium v zachyceném rozsahu"
    renameLine(KRK_RE,   'Krk v zachyceném rozsahu: ');
    renameLine(BRICHO_RE,'Epigastrium v zachyceném rozsahu: ');
  } else if (count === 1 && b) {
    // jen BŘICHO: Hrudník -> "Hrudním v zachyceném rozsahu", smazat blok Krk až po Hrudník (Hrudník ponechat)
	t = t.replace(/(^|\r?\n)Krk:[\s\S]*?(?=\r?\nHrudník:\s)/, '$1');
    renameLine(HRUDNIK_RE, 'Hrudník v zachyceném rozsahu: ');
  } else if (count === 2 && k && h) {
    // KRK + HRUDNÍK: Břicho -> "Epigastrium v zachyceném rozsahu"
    renameLine(BRICHO_RE, 'Epigastrium v zachyceném rozsahu: ');
  } else if (count === 2 && h && b) {
    // HRUDNÍK + BŘICHO: Krk -> "v zachyceném rozsahu"
    renameLine(KRK_RE, 'Krk v zachyceném rozsahu: ');
  }
  // 0 nebo 3 zaškrtnuté: bez změny

  // kosmetika: sloučit vícenásobné prázdné řádky
  t = t.replace(/\n{3,}/g, '\n\n');
...
if (containsBanned) {
    ...
    if (buttonElementPETType.value === "FDG") {
        RESTextNative = "Bez patrné patologie. ";
...
RESNeckLesion3 + "\n" +
RESNeckLymphNode1 + "\n" +
NeckOther1ResPriority + " " + NeckParotidRes + " " + NeckVCordsRes + "\n" +
RESThoraxLesion1 + "\n" +
RESThoraxLesion2 + "\n" +
RESThoraxLesion3 + "\n" +
RESThoraxLymphNode1 + "\n" +
