...
if (!window.vertebraLabels) window.vertebraLabels = {};
function setDynamicWVars() {
...
const defaultDiscLabel = { X01:"T12/L1", X12:"L1/2", X23:"L2/3", X34:"L3/4", X45:"L4/5", X56:"L5/S1" };
const defaultVertLabel = { X0:"T12", X1:"L1", X2:"L2", X3:"L3", X4:"L4", X5:"L5", X6:"S1" };

function discLabel(did){
  return (window.vertebraLabels && window.vertebraLabels[did]) || defaultDiscLabel[did] || did;
}
function rootUp(did){       // horní kořen k disku Xab ⇒ Xa
  const a = parseInt(did[1],10);
  const vid = "X"+a;
  return (window.vertebraLabels && window.vertebraLabels[vid]) || defaultVertLabel[vid] || "";
}
function rootDown(did){     // dolní kořen k disku Xab ⇒ Xb
  const a = parseInt(did[1],10)+1;
  const vid = "X"+a;
  return (window.vertebraLabels && window.vertebraLabels[vid]) || defaultVertLabel[vid] || "";
}
...
if (typeof window.rebuildVertebraLabelsFromSelects === 'function') {
  window.rebuildVertebraLabelsFromSelects();
}
setDynamicWVars();
...
if (LORDText === "(") {
 Osy += "Paradoxní lordotizace. "; 
  OsyR += "Paradoxní lordotizace hrudní páteře. ";
} else if (LORDText === "|") {
 Osy += "Kyfóza vyrovnaná. ";
 OsyR += "Napřímená hrudní kyfóza. ";
} else if (LORDText === ")") {
 Osy += "Kyfóza Tp přiměřená. ";
} else if (LORDText === "))") {
 Osy += "Kyfóza Tp akcentovaná. ";
} 
...
function getVertebraLabel(id) {
  return (window.vertebraLabels && window.vertebraLabels[id]) ? window.vertebraLabels[id] : "";
}
...
`;
// Z původní šablony pro X01 uděláme varianty s dynamickými popisky
function adaptCode(baseCode, discId){
  // horní/dolní obratel k danému disku (X01→X0/X1, X12→X1/X2, ...)
  const upId   = "X" + parseInt(discId[1],10);
  // const dnId = "X" + (parseInt(discId[1],10)+1); // nepotřebujeme přímo

  return baseCode
    // přemapování identifikátorů X01 → X12/X23... (logika if/else atd.)
    .replace(/X01/g, discId)
    // listéza vychází z horního obratle (X0→X1 pro X12 atd.); \b brání zásahu do X01
    .replace(/\bX0(?!1)/g, upId)
    // textové štítky úrovně: "Level0/1: " => discLabel("X..")+": "
    .replace(/"Level0\/1:\s*"/g, 'discLabel("'+discId+'") + ": "')
    // kořeny: Root0/Root1 uvnitř vět nahraď dořetězením dynamických názvů
    .replace(/Root0/g, '" + rootUp("'+discId+'") + "')
    .replace(/Root1/g, '" + rootDown("'+discId+'") + "');
}

// vytvoř dynamické varianty a spusť
let codeForX01D = adaptCode(codeForX01, "X01");
let codeForX12  = adaptCode(codeForX01, "X12");
let codeForX23  = adaptCode(codeForX01, "X23");
let codeForX34  = adaptCode(codeForX01, "X34");
let codeForX45  = adaptCode(codeForX01, "X45");
let codeForX56  = adaptCode(codeForX01, "X56");
...
if (X01nativR === "" && X12nativR === "" && X23nativR === "" && X34nativR === "" && X45nativR === "" && X56nativR === "" &&
    ...
    LLnormalR = "Přiměřený nález na hrudní páteři.";
} else {
    LLnormalR = "";
}
